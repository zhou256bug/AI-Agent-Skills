...
def transform(text: str, *, period: int, summary_date: str) -> str:
    text = re.sub(
        r"汇报周期：[^\n]+",
        f"第{period}期 | 汇总日期：{summary_date} | 来源：陈徐伟",
        text,
        count=1,
    )
    # Drop section 一、菜头… through before 二、重点项目
    text = re.sub(
        r"## 一、菜头（蔡伟旭）本周重点\n\n.*?(?=## 二、重点项目进展)",
        "",
        text,
    ...
    text = text.replace("## 四、菜头业务线重点关注", "## 三、业务线重点关注")
    text = text.replace("## 五、需菜头关注的事项", "## 四、需菜头关注的事项")
